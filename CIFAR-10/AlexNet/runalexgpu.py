...
def mini_alexnet(images):
    #conv1
    with tf.name_scope('conv1') as scope:
        kernel=tf.Variable(tf.truncated_normal([5,5,3,64],dtype=tf.float32,
                                            stddev=1e-1),name='weights')
        conv=tf.nn.conv2d(images,kernel,[1,1,1,1],padding='SAME') # stride=4
        print('conv shape',conv.shape) #(?, 32, 32, 64)
    ...
    print('pool1 shape',pool1.shape) #(?, 15, 15, 64)
    ...
    with tf.name_scope('conv2') as scope:
        ...
        print('conv2 shape',conv2.shape)#(?, 15, 15, 64)
    ...
    with tf.name_scope('lrn2') as scope:
        lrn2 = tf.nn.local_response_normalization(conv2,alpha=1e-4,beta=0.75,
                                                depth_radius=2, bias=2.0)
    # pool2
    pool2 = tf.nn.max_pool(lrn2, ksize=[1, 3, 3, 1],strides=[1, 1, 1, 1],
...
def inference_alexnet(images):
    ...
    with tf.name_scope('conv2') as scope:
        ...
        bias = tf.nn.bias_add(conv, biases)
        conv2 = tf.nn.relu(bias, name=scope)
        print('conv2 shape',conv2.shape) #(?, 27, 27, 128)
    tf.summary.histogram('Convolution_layers/conv2',conv2)
    tf.summary.scalar('Convolution_layers/conver2',tf.nn.zero_fraction(conv2))
    #lrn2
    with tf.name_scope('lrn2') as scope:
    ...
    pool2 = tf.nn.max_pool(lrn2, ksize=[1, 3, 3, 1],strides=[1, 2, 2, 1],
                            padding='VALID',name='pool2')
    print('pool2 shape',pool2.shape) # 13 13 128
    #conv3
    with tf.name_scope('conv3') as scope:
        ...
        bias=tf.nn.bias_add(conv,biases)
        conv3=tf.nn.relu(bias,name=scope)
        print('conv3 shape',conv3.shape) # 13 13 256
    tf.summary.histogram('Convolution_layers/conv3',conv3)
    tf.summary.scalar('Convolution_layers/conver3',tf.nn.zero_fraction(conv3))
    #conv4
    with tf.name_scope('conv4') as scope:
        ...
        bias=tf.nn.bias_add(conv,biases)
        conv4=tf.nn.relu(bias,name=scope)
        print('conv4 shape',conv4.shape) # 13 13 256
    tf.summary.histogram('Convolution_layers/conv4',conv4)
    tf.summary.scalar('Convolution_layers/conver4',tf.nn.zero_fraction(conv4))
    #conv5
    with tf.name_scope('conv5') as scope:
    ...
    pool5=tf.nn.max_pool(conv5,ksize=[1,3,3,1],strides=[1,2,2,1],
                            padding='VALID',name='pool5')
    print('pool5 shape',pool5.shape) # [None, 6, 6, 64]
    ...
    #fully_connected2
    with tf.name_scope('fully_connected') as scope:
        weights =tf.Variable(tf.truncated_normal([4096,2048],dtype=tf.float32,
                                                stddev=1e-1),name='weights')
        biases=tf.Variable(tf.constant(0.0,shape=[2048],dtype=tf.float32),
                                        trainable=True,name='biases')
        local4=tf.nn.relu(tf.matmul(local3,weights)+biases,name=scope)
    tf.summary.histogram('Fully connected layers/fc2',local4)
    tf.summary.scalar('Fully connected layers/fc4',tf.nn.zero_fraction(local4))
    ...
    #output
    with tf.name_scope('output') as scope:
        weights =tf.Variable(tf.truncated_normal([2048,FLAGS.class_num],dtype=tf.float32,
                                                stddev=1e-1),name='weights')
        biases=tf.Variable(tf.constant(0.0,shape=[FLAGS.class_num],dtype=tf.float32),
                                        trainable=True,name='biases')
        softmax_linear=tf.add(tf.matmul(local4,weights),biases,name=scope)
...
def build_model(device):
    ...
    with tf.device(device):
        ...
        predictions = tf.nn.softmax(out)
        top_1_op = tf.reduce_mean(tf.cast(tf.nn.in_top_k(predictions, labels, 1),tf.float32))
        top_5_op = tf.reduce_mean(tf.cast(tf.nn.in_top_k(predictions, labels, 5),tf.float32))
        ema = tf.train.ExponentialMovingAverage(0.9, global_step)
        tf.add_to_collection('resnet_update_ops', ema.apply([loss]))
        decay_steps = int(50000/FLAGS.batch_size)
        boundaries = [round(20 * decay_steps), round(35 * decay_steps)]
        learing_rates = [0.001, 0.001, 0.001]
