...
def image_alexnet(images):
    ...
    with tf.name_scope('fully_connected1') as scope:
        ...
        print('dim',dim)#2304
        weights =tf.Variable(tf.truncated_normal([dim,1024],dtype=tf.float32,
                                                stddev=1e-1),name='weights')
    ...
    with tf.name_scope('fully_connected') as scope:
        weights =tf.Variable(tf.truncated_normal([1024,1024],dtype=tf.float32,
                                                stddev=1e-1),name='weights')
        biases=tf.Variable(tf.constant(0.0,shape=[1024],dtype=tf.float32),
                                        trainable=True,name='biases')
    ...
    with tf.name_scope('output') as scope:
        weights =tf.Variable(tf.truncated_normal([1024,FLAGS.class_num],dtype=tf.float32,
                                                stddev=1e-1),name='weights')
